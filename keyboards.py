...
def generate_category_menu():
    ...
    markup.row(
        InlineKeyboardButton(text='MENU', url='https://telegra.ph/Fast-food-09-25-5')
    )
...
def generate_product_detail_menu(product_id, category_id):
    ...
    markup.add(*buttons)
    markup.row(
        InlineKeyboardButton(text='🔙 Ortga', callback_data=f'back_{category_id}')
    )
    return markup
...
def generate_cart_menu(cart_id):
    ...
    markup.row(
        InlineKeyboardButton(text='✅ Tasdiqlash', callback_data=f'order_{cart_id}')
    )
